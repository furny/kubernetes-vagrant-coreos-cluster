...
MASTER_YAML = File.join(coreOsClusterPath, "master.yaml")
NODE_YAML = File.join(coreOsClusterPath, "node.yaml")
...
MOUNT_POINTS = YAML::load_file(File.join(coreOsClusterPath, "synced_folders.yaml"))
  ...
  (1..(NUM_INSTANCES.to_i + 1)).each do |i|
    ...
    config.vm.define vmName = hostname do |kHost|
      ...
      if vmName == "master"
        kHost.trigger.before [:up, :provision] do
          ...
          system <<-EOT.prepend("\n\n") + "\n"
             ...
             #{coreOsClusterPath}/setup.tmpl > #{coreOsClusterPath}/temp/setup
             chmod +x #{coreOsClusterPath}/temp/setup
          EOT
          info "Making sure 'kubectl' matches the Kubernetes version we just bootstrapped..."
          system File.join(coreOsClusterPath, "./temp/setup") + " install"
        end
      ...
      kHost.trigger.before [:destroy] do
        system <<-EOT.prepend("\n\n") + "\n"
          rm -f #{coreOsClusterPath}/temp/*
        EOT
      end

      if SERIAL_LOGGING
        logdir = File.join(coreOsClusterPath, "log")
